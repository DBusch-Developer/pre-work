...
Adding both anagram tests + impls to my repo. They don't ship anywhere — just sit as a vivid TDD demonstration I can point at next time someone asks "why bother writing tests first?"`}]}]},{number:27,label:`Day 2`,title:`Week 7, Day 2 — UPDATE & DELETE: Closing the CRUD Loop with Server Actions`,subtitle:`TDD a Server Action before building it. Wire UPDATE into the UI. Add DELETE with confirmation. Two API-route paths converted to Server Actions side-by-side. Plus README and WHY comments.`,color:`#9333ea`,sections:[{heading:`Exercise 1 — Tests First: Spec Out UPDATE Before You Build It`,description:`Pattern 2 applied to a real mutation. Four failing Vitest tests for updateAssignment, all RED. Then the implementation that makes them green.`,qa:[{q:`Setup — mocking both Prisma and next/cache`,a:`app/assignments/actions.test.js opening lines:
...
The Prisma mock extends yesterday's findMany-only version to include update. Same shape as Day 1, just one more method.`},{q:`Four tests, all RED before any implementation`,a:`test('updates an assignment title when given a valid id and new title', async () => {
...
All four RED on save: the import of updateAssignment resolves to undefined, every call throws. That's the spec, locked in before I'd written a line of the function.`},{q:`The implementation — what the failing tests forced me to decide`,a:`// Server Actions are used here instead of API routes: no fetch, no JSON serialization,
...
Saved. All four green. Total time RED-to-GREEN: about three minutes.`},{q:`Reflection — what the tests forced me to decide that I would have skipped`,a:`Two things I would have skipped.
...
I kept the API-route version of GET (single assignment) at app/api/assignments/[id]/route.js — that's useful for future client-side fetch paths. But for the WRITE path, Server Action is the cleaner answer.`},{q:`Edit page — server component, pre-fills via direct Prisma call`,a:`app/assignments/[id]/edit/page.js (key bits):
...
This is the "server outer, server outer, server outer" pattern — there's no client component anywhere in this page. The page renders on the server, the form posts to a Server Action on the server, the redirect happens on the server. ZERO JS shipped for the edit flow.`},{q:`The inline 'use server' inside the page component — why?`,a:`Could have called updateAssignment directly from the form's action attribute:
...
Could have hardcoded the redirect inside updateAssignment, but then the action couldn't be reused for places that don't want to redirect (like the inline delete on the list page).`},{q:`Cancel button — the gotcha I almost hit`,a:`Initially I had <button type="button">Cancel</button> with an onClick handler... which doesn't work on a server component. You can't put onClick on a server component (no event handlers).
...
Going to keep this pattern in mind. "Is this button doing an action or is it doing a navigation?" If it's navigation, use Link. Most "buttons" in CRUD apps are actually navigation.`},{q:`Tested the UPDATE flow end-to-end`,a:`1. Loaded /assignments/3 (Hello World Program from CS101)
...
This is what people mean when they say Next.js Server Actions "feel like Rails." The whole thing is request-response. No client state to manage, no race conditions, no loading states to wire up.`}]},{heading:`Exercise 3 — Add DELETE (Confirmation Dialog Required)`,description:`Added DELETE handler to /api/assignments/[id]/route.js. Two-state confirm + actual delete + list refresh.`,qa:[{q:`DELETE handler with P2025 catch`,a:`app/api/assignments/[id]/route.js (the DELETE export):
...
The await params is the Next.js 15+ pattern — params is a Promise that must be awaited before destructuring. Same as the GET in this same file.`},{q:`Why I scoped the Codex CLI session and what I asked for`,a:`Prompt I used:
...
The bad version of this prompt would have been "add a DELETE route." That would have gotten me a full file rewrite with imports I didn't ask for, validation I don't need, and inconsistent style with the existing code. Specificity in, specificity out.`},{q:`Wired the DELETE through Codex CLI for the UI then refactored to a Server Action (Exercise 3b)`,a:`Initial implementation: button on each list row that called fetch(\`/api/assignments/\${id}\`, { method: 'DELETE' }) inside an onClick handler with a window.confirm guard.
...
Converted to a Server Action variant (Exercise 3b path) and the code halved. See next answer for the contrast.`},{q:`Testing the DELETE flow including the Cancel path`,a:`Cycled through all four scenarios:
...
Real catch from the testing pass. Going in the cli-session-log.md as a known issue.`}]},{heading:`Exercise 3b — Convert DELETE to a Server Action`,description:`Did the optional conversion. Built deleteAssignment as a Server Action with form-based confirm. Compared shape side-by-side with the API-route version.`,qa:[{q:`The Server Action — three lines after the action signature`,a:`In app/assignments/actions.js:
...
The isNaN guard is the same shape as updateAssignment. Both actions parse an id from FormData. Both need to handle the null case. Same five lines in both functions — could be a shared helper, but premature for two callers.`},{q:`Wiring it into the list — inline form, no fetch, no useState for removal`,a:`In app/assignments/assignment-list.js, inside the map over filtered assignments:
...
Why this is a client component: confirm() is a browser API, setAssignments is React state. Both require 'use client'. Couldn't move this to pure server-side without losing the confirm dialog and the instant feedback.`},{q:`Side-by-side comparison — API route vs Server Action`,a:`API-route version (the original):
...
I kept the API route version in the codebase for now. Reasoning: programmatic deletion (a future "bulk cleanup" feature, or a CLI script) might prefer the API. The cost of keeping both is one extra file. Not worth the cleanup work yet.`},{q:`Why I chose Server Actions — the one-sentence comment on actions.js`,a:`Top of app/assignments/actions.js:
...
The choice in plain English: for any mutation that's triggered by a user form on a page in my app, Server Actions are simpler. For any mutation that needs to be callable from outside the app (programmatic clients, mobile apps, public API), API routes are the right tool. Canvas Daily is a personal app with no external clients today, so most mutations want Server Actions.`}]},{heading:`Exercise 4 — CRUD Checkpoint`,description:`End-to-end verification: create, read, update, delete. Full cycle.`,qa:[{q:`CREATE — went past the lab and built /assignments/new`,a:`The lab assumes CREATE was wired in Week 6. Canvas Daily's CREATE flow on Day 4 of Week 6 was for User profiles (the Settings page), not Assignments. So I needed to build the Assignment CREATE flow today.
...
This was extra work beyond the lab but it filled a real gap. Canvas Daily had no UI for creating assignments before today. Now it does.`},{q:`Read — already in place from Week 6 Day 3`,a:`Nothing new on the READ side. The list page, the detail page, and the related-assignments section all came from Day 3. The /api/assignments and /api/assignments/[id] routes also already existed.

What I added today on the read side: an Edit button on the assignment detail page (links to /assignments/[id]/edit) and a Delete button on each list row. Those are technically navigation/action UI additions, not new READ functionality, but they make the existing READ pages capable of branching into UPDATE and DELETE.`},{q:`UPDATE — full cycle verified`,a:`1. /assignments → list of 10 assignments
...
All works. Single round-trip. No loading spinner because the Server Action redirect happens server-side.`},{q:`DELETE — full cycle verified`,a:`1. /assignments → list of 10 assignments
...
Both paths working. The optimistic update + revalidatePath combo gives instant UX without losing source-of-truth correctness.`},{q:`Full checklist — all four operations pass`,a:`CREATE: ✓ Click + New, fill form, submit, redirect to list, new item visible
...
Canvas Daily is now a working CRUD app on Assignments. Just over two weeks since I started Week 6 with an empty Next.js scaffold.`}]},{heading:`Exercise 5 — Write Your README`,description:`Wrote a README.md that's useful for humans and for future AI sessions. Includes the data model for the Phase 2 gate verbal rehearsal.`,qa:[{q:`The README sections I included`,a:`Sections:
...
Final README is ~70% AI structure, ~30% my prose, but every sentence reads like something I'd say. That's the right ratio for this kind of doc — AI handles the boilerplate, I add the judgment.`}]},{heading:`Exercise 6 — Code Comments That Explain WHY`,description:`Added comments across the key files. Schema, actions, list component, route handler. Each comment justifies a CHOICE, not the syntax.`,qa:[{q:`Prisma schema — three model-level WHY comments`,a:`In prisma/schema.prisma, added one block comment above each model:
...
Future me (or future contributor) reading the schema knows the REASONING. If they see "this model needs updatedAt!" they'll find the comment that says "we only edit titles for now" and either change the comment (if they're adding more editable fields) or back off. The comment captures the moment-in-time judgment.`},{q:`actions.js — one comment that justifies the whole file`,a:`Top of app/assignments/actions.js:
...
This is the comment I'm proudest of. It's small but it punches above its weight. Without it, a future reader sees a file full of mutations and wonders why they aren't API routes. With it, they immediately understand the trade and can decide whether to follow the pattern or push back.`},{q:`assignment-list.js — explains the client/server tradeoff`,a:`Top of app/assignments/assignment-list.js:
...
A comment like this prevents the dumbest possible future change: someone seeing 'use client' at the top, thinking "shouldn't this be a server component?", and switching it without understanding the keystroke cost. The comment is a tiny piece of defensive engineering.`},{q:`[id]/route.js — explains why this file still exists alongside actions.js`,a:`In app/api/assignments/[id]/route.js:
...
Total comment count across the project: 11 WHY-comments in 4 files. Past the lab's "at least 5" deliverable, all earning their keep.`}]},{heading:`Peer Activity — Verbal Mini-Demo`,description:`2 minutes walking partner through the UPDATE flow. Then partner tries to break DELETE.`,qa:[{q:`My 2-minute UPDATE walkthrough`,a:`Screen-shared canvas-daily at /assignments. Pointed at the assignment list, hovered the row for "Hello World Program" — pointed out the Delete button on the right side, then clicked the row itself to navigate to the detail page.
...
Ended at the detail page with the new title visible. ~2 minutes.`},{q:`Partner tried to break DELETE — what they did and what happened`,a:`Five attempts. Two found real issues.
...
5. Network drop simulation (DevTools: throttle to Offline) → Click Delete → OK
   ...
   - REAL GAP. The optimistic update needs to revert on action failure. Currently it just trusts the action succeeded.`},{q:`The two gaps surfaced and the fixes`,a:`Gap 1: Network failure during DELETE leaves UI inconsistent.
...
Adding both to the cli-session-log.md as "known issues, fix before Week 8 gate." The peer review caught them. That's exactly what the exercise format is FOR — fresh eyes break things in ways my own eyes had stopped seeing.`},{q:`What this peer session surfaced beyond the bugs`,a:`Two meta-observations.
...
Going to do these breakdown sessions before every demo from now on. Not just at peer review.`}]},{heading:`Coding Challenge — Bookmarks Manager (#14)`,description:`Optional. The bookmarks-manager challenge as a fresh-substrate CRUD app. Scoped to UPDATE + DELETE extensions since those are what I just built.`,qa:[{q:`Why this one fit the day`,a:`Bookmarks Manager is a parallel full-stack CRUD app. The bank version covers READ and CREATE; the variations at the bottom call out UPDATE and DELETE as extensions. That's exactly what I just built in canvas-daily, in a different domain.
...
Scoped it tight: implement UPDATE + DELETE only, with TDD on the Server Actions. Started a new project (npx create-next-app), seeded a Bookmark model in Prisma, ran the same workflow as today's lab.`},{q:`TDD on the bookmarks Server Actions`,a:`Wrote five tests for updateBookmark and deleteBookmark in actions.test.js BEFORE writing the actions. Same pattern as Exercise 1 — vi.mock the Prisma client, mock next/cache, beforeEach clearAllMocks.
...
Comparing to today's canvas-daily work, where I scaffolded updateAssignment from scratch with no priors — the bookmarks version took maybe 1/4 the time. Same pattern, second rep. Faster.`},{q:`Where the second build diverged from canvas-daily`,a:`Three places.
...
Third — running the challenge as a second build is qualitatively different from doing it as my first. The first build is "learn the pattern by struggling with it." The second build is "apply the pattern as a tool." Both have value, but they teach different things. Going to start using the challenges this way — not as standalone practice, but as same-day re-runs of whatever the lab just taught.`},{q:`Logged it as session #N in cli-session-log.md`,a:`Bookmarks UPDATE/DELETE rep noted in the cli-session-log.md as session #N (just past 10 logged sessions now). Same template — goal, prompt, what came back, decision, result, what I'd do differently.
...
The phase 2 gate at end of Week 8 is two weeks out. By then I'll have logged 15+ sessions. The pattern is the artifact.`}]}]},{number:28,label:`Day 3`,title:`Week 7, Day 3 — Third-Party APIs + AI Integration`,subtitle:`Server-side Canvas proxy with timeout + error isolation. Groq + Llama 3.3 70B via the OpenAI SDK. Structured system message. AI study breakdown wired into the assignment detail page.`,color:`#0284c7`,sections:[{heading:`Exercise 1 — Explore a Public API (Core)`,description:`Picked Canvas LMS itself. The reason for canvas-daily existing in the first place — and the wall I hit back in Week 5 Day 4.`,qa:[{q:`Why I picked Canvas LMS instead of one from the suggested list`,a:`The lab's options are great practice APIs — Open-Meteo, Dog CEO, PokeAPI, JSONPlaceholder. All would work for the exercise. None of them belong in Canvas Daily.
...
It's also a way of making the "core path" feel like real work. Picking Dog CEO would have taught me the same pattern in 10 minutes. Picking Canvas teaches me the same pattern AND closes the gap that's been open in my project since Week 5.`},{q:`The four things from the docs`,a:`API: Canvas LMS REST API (Instructure)
...
The naming mismatch between Canvas's "name" and my schema's "title" is one of the small annoyances of integrating with a real API I don't control. My route handler does the rename in the response mapping (a.title = a.name).`},{q:`First call from the terminal — confirming the token worked`,a:`Before writing the Next.js route, I tested the token directly from the terminal:
...
This 30 seconds of curl validation before touching the Next.js code is the highest-leverage habit I've picked up this term. If the curl fails, no amount of Next.js plumbing will save me. If the curl succeeds, I know the upstream is fine and any failures are in MY code.`}]},{heading:`Exercise 2 — Integrate into Your App (Core)`,description:`Built /api/canvas as a server-side proxy with Bearer token in .env, plus /canvas page that fetches and renders the live data.`,qa:[{q:`The proxy route — what it does and why it has to be server-side`,a:`app/api/canvas/route.js (key bits):
...
That's the data-flow pattern in three bullets. Without the server route, none of these work. With it, all three are solved by ONE file.`},{q:`Two fetches per request — courses then assignments-per-course`,a:`Canvas's API doesn't have a "give me all my assignments across all courses" endpoint. The shape is course-scoped: /api/v1/courses/{courseId}/assignments. So I have to fetch courses first, then fan out one assignment fetch per course.
...
The PER-COURSE try/catch is the part I'm proud of. If one course's assignments fetch times out or 500s, that course renders with an empty assignments array, but the page still shows the other five courses with their data. Without the per-course catch, one bad upstream response would propagate up to the outer catch and the whole page would render as "Canvas error." The right failure mode here is partial degradation, not total failure.`},{q:`The page — client component with useEffect + fetch (and the small but real loading state)`,a:`app/canvas/page.js:
...
If the user navigates away and back, the fetch runs again. For "live Canvas data" that's the right behavior — I want fresh data on every visit, not cached.`},{q:`Secured the key — .env.example committed, .env gitignored`,a:`.env (real, gitignored):
...
git diff confirmed .env doesn't show up in any commit. Going to be paranoid about this — leaked keys are one of the most common ways junior devs get burned. Especially Canvas tokens, which have ACCOUNT-level access to a real LMS where my grades live.`},{q:`Tested it end-to-end against real Canvas`,a:`1. npm run dev
2. Visited /canvas
...
This is the thing that's been a hole in canvas-daily since Week 5 Day 4. Six days of bootcamp later, I have the architectural pieces to solve it correctly. The CORS+auth wall is gone — not by working around it, but by going through it via the proper server-side proxy pattern.`}]},{heading:`Exercise 3 — Handle Errors Gracefully + Timeouts (Stretch)`,description:`Added AbortSignal.timeout(5000) and split the catch into TimeoutError-specific 504 vs generic 503. Plus per-course try/catch from Exercise 2.`,qa:[{q:`AbortSignal.timeout — the modern primitive over manual AbortController`,a:`const TIMEOUT_MS = 5000
...
AbortSignal.timeout collapses all of that into one line. Built into Node 18+ and modern browsers. The runtime handles the timer lifecycle. Recognize the older pattern when you see it in tutorials and prefer this one.`},{q:`TimeoutError vs generic Error — different status codes for different failures`,a:`} catch (error) {
...
That's the difference between "code that runs" and "code that ships." Real users WILL trip every one of these failure modes eventually. Handling them up front is cheaper than handling them after a support ticket.`},{q:`The bonus per-course isolation that the lab didn't ask for`,a:`The lab's error handling spec covered the OUTER try/catch — wrap the whole route, return 504/503. My route has that AND a per-course try/catch around the assignments-per-course fetches.
...
This is the kind of error handling that earns its keep when production traffic hits real flaky upstreams. Canvas's per-course endpoints have noticeably variable latency depending on the course size. Building in tolerance for that variability from day one is way cheaper than retrofitting it after a support ticket.`}]},{heading:`Exercise 4 — Set Up the OpenAI API and Make Your First Call (Core)`,description:`Built /api/ai with the OpenAI SDK pointed at Groq's OpenAI-compatible endpoint. Llama 3.3 70B. First call worked, response logged, route ready for the system message in Exercise 6.`,qa:[{q:`Why I used Groq + Llama 3.3 70B instead of OpenAI`,a:`Three reasons. None of them is about provider preference.
...
This is NOT a provider comparison study. I haven't seriously measured cost, quality, or speed against OpenAI for this specific use case. I just picked the provider that minimized signup friction and let me get to the actual learning. Going to call out in the README that the provider choice is interchangeable.`},{q:`The route — clean, three-block shape`,a:`app/api/ai/route.js:
...
Notice the optional chaining on completion.choices[0]?.message?.content with the ?? fallback. If the model returns a malformed response (which it might under rate limiting), I don't crash — I return "No response generated" and the user sees something useful. Defensive, but cheap.`},{q:`First call — what worked and what didn't on the first try`,a:`Wired up a tiny test in the browser console while developing — fetch('/api/ai', { method: 'POST', body: JSON.stringify({ assignment: { title: 'Hello World', courseName: 'CS101' } }) }) — to skip the UI plumbing and test the route in isolation.
...
Total time from "open route file" to "first working response": about 8 minutes. Compared to Week 5 when I was wrestling with the same shape of code for fetch+parse+state, this felt easy. The pattern is the same; the upstream just changed.`}]},{heading:`Exercise 5 — Build a Simple Chat UI (Stretch — partially)`,description:`Skipped the standalone chat UI. The integrated AssignmentBreakdown button on the detail page IS the chat UI, in context.`,qa:[{q:`Why I didn't build a separate /ai-chat page`,a:`The lab's chat UI is generic — text input, submit, response box. Useful for proving the AI route works. NOT useful as a feature in my actual product.
...
This is a judgment call about where in the lab's stretch path to invest time. The standalone chat UI is good practice for plumbing. The integrated feature is good practice for product design. I picked product design.`},{q:`If I had built the standalone chat — what it would look like`,a:`For completeness, the version I'd build if I came back to it:
...
If I ever want this — for product debugging, for example, "let me ask the AI directly without an assignment context" — I'd add it. Not today.`}]},{heading:`Exercise 6 — Add a System Message (Core)`,description:`Custom system message tailored to canvas-daily: study-assistant role, structured markdown output with four named sections, 300-word cap.`,qa:[{q:`The system message — structured output is the entire point`,a:`content: \`You are a study assistant for a college student. Given an assignment, produce a clear, actionable breakdown in this exact format:
...
The "be direct — the student needs to get to work, not read an essay" closing line is the personality dial. Sets tone WITHOUT being a long-winded persona instruction. One sentence, ends the system message.`},{q:`Why temperature 0.5 (not the default 0.7 or 1)`,a:`temperature: 0.5
...
0.5 is the right point on the trade curve for this specific use case. Going to revisit if I add more variety-seeking AI features.`},{q:`Before-and-after: same assignment, no system message vs my system message`,a:`Tested with assignment: { title: "Final Essay on Industrial Revolution", courseName: "HIS 101", pointsTotal: 100, description: "Write a 5-page essay analyzing the social impact of the Industrial Revolution." }
...
The system message is doing 90% of the work here. The user prompt is just delivering data; the system message defines the shape of the answer.`},{q:`What I'd tell a partner about the system message in the verbal demo`,a:`Two sentences for the demo:
...
The system message is the part of an AI feature where engineering effort compounds the most. Five minutes spent tightening it improves every future response. Five minutes spent prettifying the UI improves nothing for accuracy.`}]},{heading:`Exercise 7 — Connect AI to Your Data (Stretch)`,description:`Two passes at this. First pass: the AI route accepts the assignment data inline as the prompt context. Second pass: built canvas-sync.js — a script that pulls real Canvas data INTO the local DB so the AI has the full assignment record to work with.`,qa:[{q:`The simple version — pass assignment data as context in the user prompt`,a:`In the AI route, before sending to the model:
...
The prompt-injection note from the lab applies here: this data is coming from MY database, originally pulled from Canvas. Canvas content is mostly trustworthy (instructors write it, students don't), so the risk is low. But if I later let users edit the description field freely, that text could contain a prompt-injection payload. Need to think about that when I add user-edit on the description.`},{q:`The bigger version — canvas-sync.js script to pull real data into the DB`,a:`Built prisma/canvas-sync.js as a one-shot script (run with node prisma/canvas-sync.js). It:
...
This took the project from "demo with seed data" to "live tool I'd actually use." Different category of thing.`},{q:`Schema additions — submissionState and needsResubmission`,a:`Added two fields to the Assignment model via a migration:
...
This is the kind of feature you can only design once you have real data flowing through. With seed data, every assignment looks the same — title, due date, points. With real Canvas data, you see "wait, half of these are already graded, why am I looking at them" and the filter becomes obvious.`},{q:`The prompt-injection consideration I'm taking seriously even though it's lab-marked 'we'll cover later'`,a:`The lab calls out this risk explicitly: if user-written data lands in the system message, an attacker could embed "ignore previous instructions and ..." in their text.
...
This is in my cli-session-log.md as a followup. The lab's "we'll cover this in detail later" is a real future bug if I don't track it.`}]},{heading:`Exercise 8 — Add a Real AI Feature to My Phase 2 App (Stretch)`,description:`Built the AssignmentBreakdown component — a button on the assignment detail page that calls /api/ai and renders the structured markdown response in-line.`,qa:[{q:`The product decision — why a breakdown, not a description-generator`,a:`The lab's example feature is "AI-generated descriptions" — user creates a recipe, AI writes the description. Useful for a recipe app.
...
Real product value, not a gimmick. The kind of feature I'd actually use on myself when CSA-250 lands a new assignment.`},{q:`The component — client component with three states (idle, loading, done)`,a:`app/components/AssignmentBreakdown.js:
...
The Reset button matters more than it looks. Without it, once the user generates a breakdown, the section is locked to that response forever. With Reset, they can dismiss it (maybe the breakdown wasn't useful, or they're done with it) and re-run if they want a fresh one. Small UX detail. Costs nothing. Worth it.`},{q:`react-markdown + prose styles — making the response render properly`,a:`The response comes back as markdown text:
...
This is the part where Tailwind earns its keep on AI features. LLMs love returning markdown. Without a markdown renderer + prose styling, every AI feature reads like a config file. With them, it reads like the kind of polished content the user expects.`},{q:`Tested it on real assignments`,a:`Ran the breakdown on three real assignments from my Canvas data:
...
This is the AI feature I'm proudest of in the whole project. Real product value, real users (me, repeatedly), real integration into the workflow. Going to demo this at the Phase 2 gate.`}]},{heading:`Peer Activity — Verbal Mini-Demo (Option 1: Data-Flow Pattern)`,description:`Picked Option 1 — the data-flow pattern. Walked partner through frontend → server route → external API, using the Canvas integration as the live example.`,qa:[{q:`Why I picked Option 1 over Option 2`,a:`Option 2 (system message walkthrough) is also a good demo — and I'd already articulated my system message thinking in Exercise 6. Saying it again to a partner would be reinforcing the same material.
...
Also Option 1 maps to my Canvas integration, which is the strongest piece of today's work. If I can walk a partner through the Canvas flow and pre-empt the "but why a server route" question with a sharp three-bullet answer, that's the rehearsal I want.`},{q:`My 2-minute walkthrough`,a:`Screen-shared canvas-daily at /canvas. Live data loaded — six courses, dozens of assignments. Pointed at the loading state ("Loading Canvas data...") that briefly showed up before the data appeared.
...
Ended on the page with the data still showing, opened DevTools, demonstrated that the Network tab shows /api/canvas as the only outgoing request — NO request to canvas.instructure.com from the browser. The proof.`},{q:`Partner's follow-up and my answer`,a:`Partner's question: What HTTP status would you return if the Canvas API key is invalid?
...
The status I'd reserve for 500 specifically: a genuine unexpected error in MY code. Bug in my response parsing. Crash in my mapping logic. Those are real 500s. An upstream rejecting my credentials is a 401, properly attributed."`},{q:`What this peer session surfaced beyond the bug-fix kind of catch`,a:`One real thing.
